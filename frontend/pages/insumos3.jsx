...
export default function Insumos() {
    ...
    // Función para leer el peso de la balanza
    const leerPesoBalanza = async () => {
        try {
            const response = await axios.get('http://localhost:5000/peso'); // Ruta de tu servidor Python
            setPesoBalanza(response.data.peso);
            setForm(prevForm => ({ ...prevForm, stock_actual: response.data.peso }));
        } catch (error) {
            console.error("Error al leer el peso de la balanza:", error);
            Swal.fire('Error', 'No se pudo conectar con la balanza. Asegúrate de que el servidor local está corriendo.', 'error');
        }
    };
